		...
		success: function(data){
			...
			if (mmsg == '') {
				...
				for(var i = 0; i < list.length; i++){
					...
					tmpstr += '<a lay-href="' + $.trim(list[i].path) + '"><i class="layfa-menu-icon ' + $.trim(list[i].icon) + '"></i><cite>' + list[i].name + '</cite></a>';
					tmpstr += '</li>';
				}
				if(catId == 'b3dc8d7901e246a98b21721eacefd9f9' && pro_all_list.length > 0){
					for(var ii = 0; ii < pro_all_list.length; ii++){
						var thisp = pro_all_list[ii];
						tmpstr += '<div class="index-nav-project" onclick="into_project(this, \'' + thisp.id + '\')">';
...
function into_project(o, project_id){
	$(o).siblings().removeClass("index-nav-project-checked");
	$(o).addClass("index-nav-project-checked");
	$('#frmMain').attr('src', CTX + '/prot/wbs/' + project_id + '/overview.html');
}