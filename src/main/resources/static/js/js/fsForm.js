...
layui.define(['layer', "fsCommon", "form", 'laydate', "fsConfig", 'layedit'],function(exports) {
    ...
    FsForm.prototype.renderDate = function() {
        ...
        $(thisForm.config.elem).find("input.fsDate").each(function() {
            ...
            var dateRang = _this.attr("dateRang"); //连接符
            options["elem"] = this; //指定元素;
            if (dateRange == "1") {
                if ($.trim(dateRang) != '') {
                    options["range"] = dateRang;
                } else {
                    options["range"] = true;
                }
            }
    ...
    FsForm.prototype.renderEditor = function(){
        ...
        var _height = $(this).attr("height") || 100;
        var id =  $(this).attr("id");
        if ($.trim(id) == ''){
            alert("请在文本编辑器添加id属性!");
            return false;
              ...
              height: _height,
                language:'zh_CN',
                 plugins: [
                  ...
                  toolbar2: "undo redo | searchreplace | bullist numlist | outdent indent blockquote | link unlink anchor image  code | insertdatetime preview",
                  ...
                  images_upload_handler: function (blobInfo, success, failure) {
                    ...
                    xhr.open('POST', $.result(fsConfig,"global.imageUrl"),'/upload/kindupload');
                    xhr.onload = function() {
                      if (xhr.status != 200) {
