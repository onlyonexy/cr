 ...
 * @License：MIT
 */
layui.use(['fsForm', 'element'],function() {
    var fsForm = layui.fsForm,
    element = layui.element;
    /********* form 表单处理   start *************/
    var formDoms = $("form");
    if (formDoms.length > 0) { //如果没有查到form，自动关闭
        $.each(formDoms,function(index, domEle) {
            fsForm.render({
                elem: $(this)
            });
        });
   ...
   *//*
    function clickCallback(e, treeId, treeNode) {
        ...
        $.each(clickCallbackIds.split(','),
        function(i, clickCallbackId) {
    ...
    *//********* datagrid 处理   start *************//*
    ...
    if (tabs.length > 0) {
        $(tabs).each(function() {
            ...
            var datagrid = fsDatagrid.render({
                id: tableId,
                clickCallBack: clickCallBack,
